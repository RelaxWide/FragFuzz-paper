...
Existing research analyzed the impact of fragmentation on performance but lacked attempts to utilize it as an attack vector for verifying firmware's extreme-situation response.
This research reverses this conventional perspective: we utilize fragmentation metrics not as targets to optimize but as objective functions for fuzzing that should be maximized, intentionally driving firmware to worst-case resource states and exposing hidden resource-exhaustion bugs and race conditions that do not manifest under typical workloads.
