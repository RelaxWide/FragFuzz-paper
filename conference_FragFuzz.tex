...
Rather than a hand-crafted, per-platform keyword list, we derive a generalizable \emph{threshold-sink pattern}: a branch comparison (\texttt{icmp}/\texttt{switch}) in which a named state variable is tested against a constant or configuration bound, and whose identifier belongs to a compact lexical family of threshold terms (e.g., \texttt{thres}, \texttt{limit}, \texttt{bound}).
...
Even so, TVDA's higher precision is not perfect: some extracted variables (\texttt{block\_id}, \texttt{pageID}, \texttt{pgs\_per\_blk}) are physical-address fields that sit near threshold comparisons but change on every I/O, acting as noise during state tracking.
