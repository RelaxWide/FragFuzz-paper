...
Coverage-guided fuzzers (CGF) such as AFL~\cite{afl} and AFL++~\cite{aflpp} save and mutate inputs that increase code coverage, but face two limitations in SSD firmware testing.
First, they are insensitive to state changes: inputs that do not immediately raise coverage are discarded, losing the accumulated I/O needed to reach thresholds and yielding coverage plateaus.
Second, I/O is nondeterministic: due to address-mapping state, multi-channel queuing, and background operations, identical input sequences may take different code paths depending on internal state, which confuses seed selection and degrades reproducibility.
...
The final FFM score is the equally weighted average of the three metrics.
Since each metric captures a different aspect of fragmentation and their relative importance varies by workload, equal weighting is a robust, bias-free default.
...
It is worth clarifying why we evaluate \emph{reachability efficiency} rather than bug count.
...
The actionable signal is therefore how quickly the fuzzer reaches the deep maintenance logic, witnessed by the near-complete datapath coverage reported in the following subsection.
As an existence proof that reaching this region exposes real defects, we uncovered a request-omission defect in FEMU's I/O error-handling path, where requests were not enqueued to the completion queue on error, causing indefinite host waits and resource leaks.
...
We note that the boundary of this subset is expert-defined and thus carries a degree of subjectivity, which we record as a threat to construct validity.
...
Our choice of threshold-based logic as the core target is grounded in SSD firmware structure.
As analyzed in Section 4.B, core maintenance logic (GC trigger, Must-GC, wear leveling, thermal and lifespan management) across major open-source platforms is, without exception, gated by threshold comparisons; firmware monitors resource states and performs costly maintenance only when these conditions are met, so tracking threshold variables and steering fuzzing toward them is an effective verification strategy.
...
The I/O-related basic-block subset used for the curated coverage metric (Section 5.C) and the ``valid'' labeling of extracted threshold variables (Section 5.D) were both determined by SSD-firmware engineers against the platforms' documented logic.
