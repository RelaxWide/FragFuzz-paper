...
To reach the underlying base variables that the fuzzer can directly control, TVDA automatically performs backward slicing on the Program Dependency Graph (PDG)~\cite{horwitz_pdg}, collecting every base variable within an upper-bound hop depth $K$.
This bound follows from the structure of SSD FTLs: from I/O request processing to the final update of global resource state, an FTL exposes five hierarchical abstraction layers (Request $\rightarrow$ Page $\rightarrow$ Block $\rightarrow$ Line/Superblock $\rightarrow$ Global Resource), so the conditions gating core maintenance logic lie at most five hops from the threshold comparison; localized logic such as wear leveling or victim-block selection sits even closer (3--4 hops).
We therefore set $K=5$, the deepest such layer, and apply it uniformly across all platforms.
