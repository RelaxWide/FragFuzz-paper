...
The key parameter is the slicing depth (Hop Depth, $K$), which TVDA fixes algorithmically as part of the automated pipeline.
...
Here $K$ is an \emph{upper bound} on the slice depth: TVDA collects every base variable within $K$ hops, so the choice is deliberately asymmetric.
...
Setting $K=5$ to match the deepest FTL abstraction layer therefore provides a conservative, structurally grounded default that captures all core maintenance logic, and we fix it across all platforms.
